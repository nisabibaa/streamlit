...
if len(df) > 1:  
    t_score, p_value = ttest_1samp(df['data'], popmean=null_mean)
    ...
    if clicked:
        ...
        pdf = t.pdf(x, df=len(df)-1, loc=df['data'].mean(), scale=df['data'].std()/np.sqrt(len(df)))
        fig, ax = plt.subplots()
        ...
        cdf = t.cdf(x, df=len(df)-1, loc=df['data'].mean(), scale=df['data'].std()/np.sqrt(len(df)))
        fig, ax = plt.subplots()
